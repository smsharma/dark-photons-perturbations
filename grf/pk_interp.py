...
class PowerSpectrumGridInterpolator():
    ...
    def __call__(self, z_ary, k_ary):
        ...
        return np.where(np.transpose(log_z_mesh_ary) <  np.log10(self.z_min), 
                        self.interpolator(np.transpose([np.ones_like(log_z_mesh_ary) * np.log10(self.z_min), log_k_mesh_ary])), 
                        self.interpolator(np.transpose([log_z_mesh_ary, log_k_mesh_ary])) )

        # return np.where(np.transpose(log_k_mesh_ary) >  np.log10(self.k_max), 
        #                 np.transpose(-100. * np.zeros_like(log_z_mesh_ary)), 
        #                 self.interpolator(np.transpose([log_z_mesh_ary, log_k_mesh_ary])) )